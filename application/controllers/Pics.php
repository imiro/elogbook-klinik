...
class Pics extends CI_Controller {
	...
	public function index($id=1)
	{
		...
		// $client->setDeveloperKey("AIzaSyCXh6tiRp71D7qBejOJKATKkzzrZJNruSw");
		...
		// echo "<pre>" . var_export($client->getAccessToken(), true) . "</pre>";

		//
		// $r = $service->files->get("13MpLPTNEWnCfPJWayXZ5ApUIPgORLs74");
		// echo $r->getName();

		//
		//    // print "Files:\n";
		//     foreach ($results->getFiles() as $file) {
		// 			 	// $r = $service->files->get($file->getId(), array('alt' => 'media'));
		// 				// header('Content-type: ' . $file->getMimeType());
		// 				// echo $r->getBody()->getContents();
		// 				echo "<pre>";
		//         echo "{$file->getName()} || {$file->getMimeType()} ({$file->getId()}) || " ;
		// 				echo var_export( $file->getParents(), true );
		// 				echo "</pre>\n";
		// 				// foreach($file->getParents() as $p) echo "$p ";
		// 				// echo "\n";
		//     }
	}

	public function viewer($fileId) {
		$client = $this->getClient(APPPATH . 'tokens.json');
		$client->setScopes( Google_Service_Drive::DRIVE );
		$service = new Google_Service_Drive($client);

		$cb = $service->files->get('1ra9M_PRnno_Xd0lNXuFFsE4mJFvNqcnu', array(
			'alt' => 'media'
		));
